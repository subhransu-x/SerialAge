...
import { ruud_STRUCTURAL_SOURCES } from './sources';

// ---------------------------------------------------------------------------
// Structural matching regex
// ---------------------------------------------------------------------------
// Modern ruud HVAC serials are structurally represented as:
...
// ---------------------------------------------------------------------------
// Format: ruud-modern-structural
// ---------------------------------------------------------------------------

const ruudModernStructural: FormatRule = {
  id: 'ruud-modern-structural',
  name: 'ruud Structural Format',
  description:
    'Modern ruud and Ruud HVAC equipment format. Extracts the manufacture date structurally ' +
  ...
  decode(input: NormalizedInput): FormatDecodeResult {
    ...
    if (/^[A-Z][0-9]{9}$/i.test(cleanStr)) {
      productType = 'unknown';
      warnings.push(
        'This 10-character format is shared between ruud HVAC equipment and ruud Water Heaters. ' +
...
// ---------------------------------------------------------------------------
// Export all ruud format rules
// ---------------------------------------------------------------------------

export const formats: readonly FormatRule[] = [
  ruudModernStructural,
];
