...
describe('Rheem Decoder (Modern Structural)', () => {
  beforeEach(() => {
    _resetRegistryForTesting();
    registerRheem();
  });
  ...
  describe('C. Specific explicit requirements', () => {
    ...
    it('Water-heater collision: M141209135 must NOT be automatically interpreted as HVAC', () => {
      const result = decode('rheem', 'M141209135', { referenceDate: REFERENCE_DATE });
      expect(result.status).toBe('success');
      expect(result.productType).toBe('unknown'); // NOT hvac
      expect(result.warnings.length).toBeGreaterThan(0);
      expect(result.warnings[0]).toContain('shared between Rheem HVAC equipment and Rheem Water Heaters');
    });
    ...
    it('Multiple structurally valid candidates are logically impossible, but verify complex string', () => {
      ...
      const result = decode('rheem', 'M1210123A45678912', { referenceDate: REFERENCE_DATE });
      expect(result.status).toBe('success');
